...
def listar_mis_reportes(
    ...
    return base_datos.scalars(
        select(Reporte).where(Reporte.usuario_id == usuario_actual.id).order_by(Reporte.creado_en.desc())
    ).all()
...
def listar_reportes(
    ...
    return base_datos.scalars(select(Reporte).order_by(Reporte.creado_en.desc())).all()
